...
\subsection{Odômetro}
\label{odometro}
O odômetro é um sensor proprioceptivo, ele mede as velocidades dos motores das rodas, e baseado nessa informação é possível determinar a posição do robô mo mapa. O principal problema dos odômetros é com relação à sua precisão, pois muitos fatores podem atrapalhar a leitura, uma roda patinando ou um pneu murcho por exemplo, o odômetro pode interpretar que o robô está fazendo um movimento diferente do real. Para corrigir esse tipo de problema existe os algoritmos de localização, que serão explicados em \cite{localizacao}.

\subsection{LASER}
\label{laser}
O LASER é um sensor Exteroceptivo Ativo, pois ele emite um sinal de luz e calculando o tempo que esse sinal demora para retornar ao LASER. Ele funciona da seguinte forma: um emissor envia o sinal e calcula o tempo que esse sinal demora para retornar ao emissor, com isso ele calcula a distância naquela direção, e através de um espelho fixo e outro rotativo é possível atingir até 360 graus de rotação.

\huge{IMAGEM DO FUNCIONAMENTO DO LASER}
...
\huge{COLOCAR CARACTERÍSTICAS TÉCNICAS?}
...
\huge{COLOCAR FIGURAS PARA ILUSTRAR O FLUXO DE CONTROLE DO PLAYER E DESCREVER MELHOR COMO FUNCIONA A INTERFACE DOS DRIVERS}
