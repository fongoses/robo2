...
Esse capítulo explicara um pouco sobre Robôs Móveis Autônomos. Um robô móvel autônomo é dotado de três características importantes: Ativadores, responsáveis, entre outras coisas pela locomoção; Sensores, que fazem o papel da percepção do robô; E controle, que aciona os ativadores de acordo com a leitura dos sensores, seguindo o propósito para qual o robô está sendo utilizado
...
\textbf{Ativos}, são sensores que emitem energia para o ambiente, então medem a reação do ambiente à essa energia. Sensores deste tipo têm uma performance superior, entretanto existem alguns riscos, como a falta de energia pode afetar a característica que o sensor está tentando medir. Sensores ativos também podem sofrer interferências de sinais que estão fora do seu controle. Por exemplo, sinais emitidos por robôs próximos, ou sensores similares no mesmo robô, podem influenciar no resultado das medidas. Exemplos de sensores ativos incluem sensores ultra-sônicos, sonares, e LASERs para medir distância \cite{Siegwart2004}. Os sensores que foram utilizados no projeto foram LASER e odômetro.

\subsection{Odômetro}
\label{odometro}
O odômetro é um sensor proprioceptivo, ele mede as velocidades dos motores das rodas, e baseado nessa informação é possível determinar a posição do robô mo mapa. O principal problema dos odômetros é com relação à sua precisão, pois muitos fatores podem atrapalhar a leitura, uma roda patinando ou um pneu murcho por exemplo, o odômetro pode interpretar que o robô está fazendo um movimento diferente do real. Para corrigir esse tipo de problema existe os algoritmos de localização, que serão explicados em \ref{localizacao}.

\subsection{LASER}
\label{laser}
O LASER é um sensor Exteroceptivo Ativo, pois ele emite um sinal de luz e calcula a distância do objeto. Ele funciona da seguinte forma: um emissor envia o sinal e calcula o tempo que esse sinal demora para retornar ao emissor, com isso é possível calcular a distância naquela direção, e através de um espelho fixo e outro rotativo é possível atingir até $360^\circ$ de rotação. A Figura \ref{fig:laser} mostra um LASER $180^{\circ}$.
...
O servidor Player foi desenvolvido para que seus clientes sejam independentes de linguagem e de plataforma. O programa cliente pode executar em qualquer máquina que tenha conexão de rede com o robô, e pode ser escrito em qualquer linguagem que suporte sockets TCP. Atualmente existem clientes disponíveis em C++, Tcl, Java e Phyton \cite{Player}. Futuramente, o Player não será indiferente sobre como o programa de controle do robô é estruturado, ou seja, poderá escrever desde programas multi-threads altamente concorrentes até programas sequências simples.

\subsubsection{Interface dos Drivers}
\label{player_interface}
Usualmente as interfaces entre o programa do usuário e o robô se dá como mostra a Figura \ref{fig:interface_usual}, onde o programa do usuário tem que fazer a aquisição de dados diretamente dos sensores e tratar esses dados de modo que o planejador compreenda. E gerar os comandos para o motor, ou outros atuadores. Sendo que para cada modelo de motor os comandos diferentes, assim como cada câmera, LASER e outros sensores tem leituras diferentes de acordo com o modelo.
...
O Player tem como função fazer essa interface entre os sensores e atuadores e o programa do usuário (Figura \ref{fig:player_interface}, lendo de qualquer modelo de um mesmo sensor (que seja aceito pelo Player) e fornecendo ao programa do usuário os dados já tratados, ou seja, para qualquer modelo de um mesmo sensor (que seja aceito pelo Player) o programa do usuário lê da mesma forma, assim como para os atuadores como mostra a Figura \ref{fig:modelo_cliente_servidor}. Por exemplo, para fazer com que o robô ande para frente na velocidade de 1 m/s, basta o programa do usuário informar a direção e velocidade, o comando vai ser o mesmo para diversos modelos de robôs, que possuem tamanhos e números de rodas diferentes, quem faz os cálculos para determinar qual a rotação da roda para atingir tal velocidade é a interface do Player, assim como para ler do sensor LASER, basta informar de qual ângulo quer ler.

\begin{figure}
	\centering
		\includegraphics[width=1.00\textwidth]{../../imagens/player2.png}
	\caption{Interface do Player}
	\label{fig:interface_player}
\end{figure}
...
Como mostrado na Figura \ref{fig:exemplo_cliente_servidor}, onde a letra 'P' indica que naquele dispositivo existe um programa Player sendo executado e a letra 'C' indica que naquele dispositivo está sendo executado um programa cliente.

\begin{figure}
	\centering
		\includegraphics[width=1.00\textwidth]{../../imagens/exemplo_cliente_servidor.png}
	\caption{Exemplo de Clentes e Servidores}
