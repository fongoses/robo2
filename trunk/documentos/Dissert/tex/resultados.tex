...
\begin{table}[ht]
	...
	\caption{Mapas para testes}
		\begin{tabular}{|c|c|c|}
			...
			\hline
			Quatro			& 4 & Iguais		 \\
			Quatro Diff	& 4 & Diferentes \\
			X						& 5 & Diferentes \\
			X Incompleto& 5 & Diferentes \\
			Espinha			& 6 & Iguais		 \\
			Espinha Diff& 6 & Diferentes \\
			Apartamento	& 8 & Diferentes \\
		\end{tabular}
...
Como o Mapa Quatro é relativamente pequeno (dois metros quadrados) e a seqüência gerada pela solução \textit{Offline} é visitar as salas no sentido anti-horário, o robô leva exatamente o mesmo tempo para ir de uma sala à outra. Como as prioridades iniciais do mapa são todas iguais e para gerar o gráfico foram tiradas medidas (Grau de Urgência Total) de dez em dez segundos, coincidiu de o robô a cada dez segundos estar no mesmo lugar no mapa, por isso o gráfico do Mapa Quatro para a solução \textit{Offline} é uma linha. 
%Fato que não acontece no gráfico do Mapa Quatro Diff (Figura \ref{fig:grafico_quatro_diff}), pois as prioridades não são todas iguais.
...
\item X: Com cinco salas que formam um X (Prioridades na Tabela \ref{tab:P_x}) com ligações entre as salas das pontas (Figura \ref{fig:grafo_x}):
